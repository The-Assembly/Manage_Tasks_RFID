...
try:
    while True:
        ...
        os.system("python3 Documents/time.py") # Running the code for the OLED screen
        print("time finished please tag out")
        while True:
            text = reader.read() # Waiiting for RFID to tag out
            if check == text[0]:
                break
            os.system("python Documents/tagout.py")
            print("Wrong RFID, use the same tag you used to tag in")
        print("tagged out")
        os.system("python Documents/tagin.py") # Running the code to display "Please tag in" on the OLED screen
        sleep(1)
